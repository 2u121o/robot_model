...
Robot::Robot(const cv::Mat &initial_map, const Eigen::Vector3d initial_state, int radius, bool add_noise): 
...
void Robot::moveRobot(const cv::Mat &map, int direction)
{
    ...
    state_before_collision_check_(0) = state_(0) + dt_*5*std::cos(state_(2))*u_v + state_noise_(0);
    state_before_collision_check_(1) = state_(1) - dt_*5*std::sin(state_(2))*u_v + state_noise_(1);
    state_(2) = state_(2) + dt_*2*u_t + state_noise_(2);
    
    if(!isCollided(map, state_before_collision_check_))
    {
        state_(0) = state_before_collision_check_(0);
        state_(1) = state_before_collision_check_(1);
    }
}

bool Robot::isCollided(const cv::Mat &map,const Eigen::Vector3d &state)
{
    int x_c = static_cast<int>(state(0));
    int y_c = static_cast<int>(state(1));
