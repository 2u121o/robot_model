...
{
    for(int k=0; k<robots_.size(); k++)
    {
        int radius = 0.0;
        robot_state_ = robots_.at(k).getStates();
        radius = robots_.at(k).getRadius();

        cv::Point center(robot_state_.x,robot_state_.y);
        cv::Scalar color(100, 0, 0);
        cv::circle(map_, center, radius, color, 1);
...
void World::drawSensorLines()
{
    for(int k=0; k<robots_.size(); k++)
    {
        robots_.at(k).getRanges(ranges_);
        robots_.at(k).getSensorSettings(sensor_settings_);
...
void World::setRobot(const std::vector<Robot>& robots)
{
    robots_ = robots;
